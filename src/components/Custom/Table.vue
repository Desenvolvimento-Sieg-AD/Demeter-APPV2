  ...
  <div class="h-100 no-smooth-behavior" v-if="render">
    ...
    >
      ...
      <DxToolbar :visible="enableAddButton || filterOn || props.enableAddButton || allowGroupingColumns || searchable || columns.some((el) => el.searchable) || chooseColumns">
        ...
        <DxItem name="reviewPayment" locate-in-menu="auto" location="after" template="reviewPayment" v-if="paymentsSelecteds" />
      ...
      <template #resetTable>
        <v-btn @click="resetTable" class="dx-button dx-button-mode-contained" color="red" flat variant="plain">
          ...
          <v-tooltip activator="parent" location="bottom" :text="`Resetar tabela`" />
          Resetar
        </v-btn>
      </template>
      ...
      <template #removingFilter>
        <v-btn @click="clearFiltersAndRefreshToolbar" class="dx-button dx-button-mode-contained" color="orange" flat variant="plain">
          <v-icon size="small" :icon="`mdi mdi-filter-remove-outline`" start />
          <v-tooltip activator="parent" location="bottom" :text="`Remover filtros`" />
          Remover filtros
        </v-btn>
      </template>

      <template #editPayment>
        <v-btn class="dx-button dx-button-mode-contained" color="primary" flat @click="editPayment" variant="tonal">
          <v-icon size="small" :icon="`mdi mdi-pencil`" start />
          <v-tooltip activator="parent" location="bottom" :text="`Editar pagamentos selecionados`" />
          Editar
        </v-btn>
      </template>

      <template #disapprovePayment>
        <v-btn class="dx-button dx-button-mode-contained" color="red" flat @click="disapprovePayment" variant="tonal">
          <v-icon size="small" :icon="`mdi mdi-close`" start />
          <v-tooltip activator="parent" location="bottom" :text="`Recusar pagamentos selecionados`" />
          Recusar
        </v-btn>
      </template>

      <template #reviewPayment>
        <v-btn class="dx-button dx-button-mode-contained" color="purple" flat @click="reviewPayment" variant="tonal">
          <v-icon size="small" :icon="`mdi mdi-close`" start />
          <v-tooltip activator="parent" location="bottom" :text="`Enviar para revisão os pagamentos selecionados`" />
          Revisar
        </v-btn>
      </template>

      <template #approvePayment>
        <v-btn class="dx-button dx-button-mode-contained" color="success" flat @click="approvePayment" variant="tonal">
          <v-icon size="small" :icon="`mdi mdi-check`" start />
          <v-tooltip activator="parent" location="bottom" :text="`Aprovar pagamentos selecionados`" />
          Aprovar
        </v-btn>
      </template>
...
const emit = defineEmits(['focusRow', 'add', 'selectionChanged', 'editPayment', 'disapprovePayment', 'approvePayment', 'reviewPayment'])
...
const editPayment = () => emit('editPayment')

const disapprovePayment = () => emit('disapprovePayment')

const approvePayment = () => emit('approvePayment')

const reviewPayment = () => emit('reviewPayment')

const onSelectionChanged = (e) => emit('selectionChanged', e.selectedRowsData)
...
watch(() => props.items, (items) => (itemList.value = items), { deep: true, immediate: true });
