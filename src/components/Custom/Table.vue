<template>
	<div class="h-100 no-smooth-behavior" v-if="render">
		...
		>
			...
			<template #removingFilter>
				<button
					@click="clearFiltersAndRefreshToolbar"
			...
			<template #lastStatus="{ data: item }">
				<div>
					{{
						item.data.movimentacoes_pagamento.length > 1
							? item.data.movimentacoes_pagamento[1].status_pagamento.nome
							: item.data.movimentacoes_pagamento[0].status_pagamento.nome
					}}
				</div>
			</template>
