<template>
	...
	<div class="h-100 no-smooth-behavior" v-if="render">
		...
		>
			...
			<DxGroupPanel
				empty-panel-text="Arraste uma coluna aqui para agrupar por essa coluna"
				:visible="allowGroupingColumns"
			/>
			...
			<DxSelection
				mode="multiple"
				:show-check-boxed-mode="checkBoxesMode"
				v-if="selectionCheck"
			...
			>
				...
				<DxItem
					...
					v-if="columns.some((el) => el.searchable)"
					locate-in-menu="auto"
					location="after"
				/>
			...
			<template #removingFilter>
				<button
					:onclick="() => clearFiltersAndRefreshToolbar()"
			...
			<DxColumn
				...
				:cell-template="
					column.value == 'actions' ? 'actions' : column.value == 'tarefas' ? 'agenda' : `item-${column.value}`
				"
			...
			>
				<DxHeaderFilter
					:data-source="column.customFilter"
					:allow-select-all="column.allowFilterSelectAll ?? true"
					:group-interval="column.dateFilter ?? null"
				>
					<DxSearch
						:enabled="column.searchFilter ?? false"
						:search-expr="column.searchFilterExpr"
						:editor-options="column.searchFilterOptions"
					/>
				</DxHeaderFilter>
			</DxColumn>
			...
			<DxRowDragging
				:allow-reordering="allowRowsReordering"
				:on-reorder="onReorder"
				:show-drag-icons="allowRowsReordering"
			/>
...
let tablePageSize = ref(
	!props.pageSize
		? props.allowedPageSizes[props.allowedPageSizes.length - 1] == 'all'
			? 0
...
const getActionDisabled = (action, item) =>
	action.disabled ? action.disabled(item.data, item.rowIndex, props.items) : false;
...
const clearFiltersAndRefreshToolbar = () => {
	customTable.value.instance.clearFilter();
...
const onOptionChanged = () => {
	...
	if (filterStorage && filterOn.value == false) {
		...
		verify =
			filterStorage.columns?.some((ref) => ref.filterValues) || filterStorage.searchText?.length > 0 ? true : false;
	}
